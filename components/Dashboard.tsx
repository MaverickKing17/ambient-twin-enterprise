...
const Dashboard: React.FC = () => {
  ...
  const runGeminiDiagnosis = async () => {
    setIsAnalyzing(true);
    try {
      const report = await getDiagnosticReport(mockTelemetry, "Standard Home Furnace");
    ...
    <div className="bg-slate-50 dark:bg-[#0b0f1a] transition-colors duration-500">
      <header className="h-20 border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-[#0b0f1a]/80 backdrop-blur-2xl sticky top-0 z-40 flex items-center justify-between px-10 shadow-sm">
        <div className="flex items-center gap-8 w-2/3">
          <div className="relative group w-full max-w-lg">
            ...
            <input 
              type="text" 
              placeholder="Search Home Systems: Serial, Address, or Name..." 
          ...
          <div className="hidden xl:flex items-center gap-3 px-4 py-2 bg-emerald-500/10 rounded-full border border-emerald-500/20">
            ...
            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-tighter">Toronto Market: Stable</span>
          </div>
        </div>
        
        <div className="flex items-center gap-8">
          <div className="flex gap-4">
             <button className="p-3 text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all relative active:scale-95">
          ...
          <div className="flex items-center gap-4 pl-8 border-l border-slate-200 dark:border-white/5">
            <div className="text-right">
              ...
              <div className="text-[9px] text-blue-600 dark:text-blue-400 uppercase tracking-widest font-black">Manager • Toronto</div>
            </div>
            <div className="relative cursor-pointer group">
              <img src="https://picsum.photos/seed/tech/80/80" className="w-12 h-12 rounded-2xl border-2 border-white dark:border-slate-800 shadow-xl group-hover:border-blue-500 transition-colors" alt="Profile" />
      ...
      <main className="p-10 max-w-7xl mx-auto space-y-12 pb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { label: 'Active Home Systems', value: '422', sub: '+12% Growth', icon: <Activity className="text-blue-600" />, color: 'bg-blue-50 dark:bg-blue-500/10' },
            { label: 'Total Home Rebates', value: '$84,200', sub: 'Ready for Review', icon: <Award className="text-amber-600" />, color: 'bg-amber-50 dark:bg-amber-500/10' },
            { label: 'Money Saved Factor', value: '3.1x', sub: 'Local Savings Data', icon: <DollarSign className="text-emerald-600" />, color: 'bg-emerald-50 dark:bg-emerald-500/10' },
            { label: 'Service Speed', value: '99.1%', sub: 'Avg 12m Response', icon: <Clock className="text-indigo-600" />, color: 'bg-indigo-50 dark:bg-indigo-500/10' }
          ].map((stat, i) => (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
          <div className="xl:col-span-2 space-y-10">
            <DigitalTwin data={mockTelemetry[mockTelemetry.length - 1]} equipment="High-Efficiency Furnace" />
            ...
            <div className="bg-white dark:bg-white/5 p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
                <div>
                  <h4 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">System Health Check</h4>
                  <p className="text-slate-500 text-sm font-medium">Real-time heating and cooling performance data.</p>
                </div>
          ...
          <div className="space-y-10">
            <div id="ai-insight-panel" className="bg-gradient-to-br from-blue-700 to-indigo-900 dark:from-blue-600/20 dark:to-indigo-900/40 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden border border-white/10">
              ...
              {!aiAnalysis && !isAnalyzing ? (
                <div className="text-center py-10 space-y-8">
                  ...
                  <p className="text-blue-100/70 text-sm font-medium leading-relaxed px-4">Summary of heating and cooling performance compared to weather data.</p>
                  <button 
                ...
                <div className="space-y-6 py-10">
                  ...
                  <p className="text-center text-xs text-blue-200/50 mt-10 font-black uppercase tracking-[0.2em] animate-pulse">Checking Data...</p>
                </div>
              ) : (
            ...
            <div className="bg-white dark:bg-white/5 p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl">
              <div className="flex justify-between items-center mb-8">
                <h4 className="text-xl font-black text-slate-900 dark:text-white tracking-tighter">Rebate Progress</h4>
                <div className="bg-emerald-500/10 text-emerald-500 text-[10px] font-black px-3 py-1 rounded-full uppercase">84% Done</div>
              </div>
              <div className="space-y-6">
                {[
                  { label: 'Energy Check', status: 'completed' },
                  { label: 'Paperwork', status: 'completed' },
                  { label: 'System Verification', status: 'active' },
                  { label: 'Getting Paid', status: 'pending' },
                ].map((step, i) => (
                  <div key={i} className="flex items-center gap-5 cursor-pointer group active:translate-x-1 transition-transform">
                    ...
                    <span className={`text-sm font-bold tracking-tight group-hover:text-blue-500 transition-colors ${step.status === 'pending' ? 'text-slate-400' : 'text-slate-900 dark:text-slate-100'}`}>
                      {step.label}
                    </span>
                  </div>
                ))}
              </div>
              <div className="mt-10 p-6 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-3xl text-center relative overflow-hidden group hover:border-emerald-500/30 transition-colors cursor-help">
                <div className="absolute top-0 left-0 w-2 h-full bg-emerald-500"></div>
                <div className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mb-2">Total Energy Rebate</div>
